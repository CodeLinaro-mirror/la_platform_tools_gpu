...
func (t Target) Build(env build.Environment) {
	...
	gapicSource := GapicRoot.Glob(t.SourceFiles...).
		Append(GapicRoot.Join(t.Gapic.OS.Name).Glob(t.SourceFiles...)...).
		Exclude("*_test.cpp")
	...
	gapiiSource := GapiiRoot.Glob(t.SourceFiles...).
		Append(GapiiRoot.Join(t.Gapic.OS.Name).Glob(t.SourceFiles...)...).
		Exclude("*_test.cpp").
	...
	gapirSource := GapirRoot.Glob(t.SourceFiles...).
		Append(GapirRoot.Join(t.Gapir.OS.Name).Glob(t.SourceFiles...)...).
		Exclude("*_test.cpp").
		// Generated by APIC
...
func base(toolchain *cpp.Toolchain, os *config.OS, architecture string) Target {
	base := cpp.Config{
		Toolchain:    toolchain,
		OS:           os,
		Architecture: architecture,
		Defines: map[string]string{
			"TARGET_OS_" + strings.ToUpper(os.Name): "1",
		},
	}
	...
	base.OutputDir = BinRoot.Join(os.Name+"-"+architecture, base.Flavor)
...
func getBuildTargets() map[string]Target {
	...
	linux := base(gcc.GCC, config.Linux, "x64").Extend(Target{
		GapicTests: cpp.Config{
	...
	osx := base(gcc.GCC, config.OSX, "x64").Extend(Target{
	...
	windows := base(gcc.GCC, config.Windows, "x64").Extend(Target{
		Gapii: cpp.Config{
			AdditionalSources: build.FileSet{
	...
	windows_msvc := base(msvc.MSVC, config.Windows, "x64").Extend(Target{
		Gapii: cpp.Config{
			AdditionalSources: build.FileSet{
				GapiiRoot.Join("windows", "opengl32_resolve.cpp")}},
		Spy: cpp.Config{
	...
	android_arm := base(ndk.APK, config.Android, "arm").Extend(android_target)
	android_arm64 := base(ndk.APK, config.Android, "arm64").Extend(android_target)
