...
TEST_F(ResourceRequesterTest, Prefetch) {
    mBuffer.resize(4096);
    EXPECT_TRUE(mResourceProvider->prefetch({{"A", 16}, {"B", 32}}, *mServer, mBuffer.data(),
                                            mBuffer.size()));
}
...
TEST_F(ResourceRequesterTest, SingleGet) {
    ...
    EXPECT_TRUE(mResourceProvider->get("A", *mServer, mBuffer.data(), 3));
...
TEST_F(ResourceRequesterTest, MultiGet) {
    ...
    EXPECT_TRUE(mResourceProvider->get({{"A", 3}, {"B", 5}}, *mServer, mBuffer.data(), mBuffer.size()));
    EXPECT_THAT(mBuffer, ElementsAreArray(payload));
    EXPECT_EQ(mConnection->out, expected);
}
}  // namespace gapir
