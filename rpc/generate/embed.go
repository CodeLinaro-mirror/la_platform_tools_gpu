...
-------------------------------------------------------------------------------
  ...
  followed by a log.Logger parameter.
-------------------------------------------------------------------------------
*/}}
{{define "ParametersAndLogger"}}
  {{AssertType $ "Function"}}
    ...
    {{if not (IsVoid $c.Return.Type)}}
      func (c client) {{$c.Name}}({{Macro "ParametersAndLogger" $c}}) (res {{Node "Type" $c.Return}}, err error) {
        ...
        if val, err = c.Send(&{{Macro "CallName" $c}}{ {{Macro "Arguments" $c}} }); err == nil {
          ...
          log.Errorf(l, "RPC {{$c.Name}} failed with error: %v", err)
        }
        return
      }
    {{else}}
      func (c client) {{$c.Name}}({{Macro "ParametersAndLogger" $c}}) error {
      ...
      switch call := in.(type) {
        {{range $i, $c := $.Externs}}
          case *{{Macro "CallName" $c}}:
            ...
            {{else}}
              if err := server.{{$c.Name}}({{range $p := $c.CallParameters}}call.{{$p.Name}}, {{end}}l); err == nil {
