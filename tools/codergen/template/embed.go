...
{{define "Cpp.CantEncode.Map"}}
// Can't encode map¶
{{$cantEncode := true}}
{{end}}

{{define "Cpp.CantEncode"}}{{end}}

{{define "Cpp.CantEncodeSchema"}}
  {{if File.Directive "Schema" true}}
    {{range .Fields}}
      {{Call "Cpp.CantEncode" .Type}}
    {{end}}
  {{end}}
{{end}}
...
{{define "Go.UpgradeClass"}}
  ...
  «}¶
  func (*{{$className}}) Decode(d binary.Decoder) binary.Object {»¶
    obj := &{{$name}}{}¶
    {{$doUpgradeDecode}}(d, obj)¶
    return obj¶
  «}¶
