...
func makeStep(name string, output build.File, source build.FileSet, always bool, a func(*graph.Step) error) *graph.Step {
	s := graph.NewStep(a)
	e := makeEntity(output)
	s.Creates(e)
	...
	if name != "" {
		graph.List("cc:" + name).DependsOn(e)
	}
	graph.List("cc").DependsOn(e)
	return s
}
...
func MakeCompile(sources build.FileSet, cfg Config, env build.Environment) build.FileSet {
	...
	for i, source := range sources {
		...
		objects[i] = object
		prev := graph.Creator(makeEntity(object))
		if prev != nil {
...
func MakeRunTest(test build.File, cfg Config) {
	...
	if cfg.ABI.OS == config.HostOS {
		...
		graph.List("cc_test").DependsOn(phony)
	}
}
