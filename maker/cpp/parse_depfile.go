...
// MakeDepsAbsolute alters the dependency paths in the specified dependency file
// by making all non-absolute paths absolute using the base path cwd.
func MakeDepsAbsolute(file, cwd string, env build.Environment) error {
	deps, ok := ParseDepFile(build.File(file), env)
	if !ok {
		return fmt.Errorf("Could not make dependencies absolute for: %v", file)
	}
	...
	for i := range deps {
		d := string(deps[i])
		if !filepath.IsAbs(d) {
			d = filepath.Join(string(cwd), d)
		}
		abs[i] = d
	}

	out := []byte(fmt.Sprintf("%s: \\\n  %s", file, strings.Join(abs, " ")))
	return ioutil.WriteFile(file, out, 0666)
}
