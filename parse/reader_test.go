...
func TestNumeric(t *testing.T) {
	for _, test := range numericTests {
		...
		if gotKind != test.outKind {
			t.Errorf("For %q git number kind %s, but expected %s.", test.in, gotKind,
				test.outKind)
		}
		if gotTok.String() != test.outToken {
			t.Errorf("For %q git token %q, but expected %q.", test.in, gotTok,
				test.outToken)
		}
	}
}
