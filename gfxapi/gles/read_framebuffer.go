...
func (t transientAtom) ContextID() atom.ContextID { return t.contextID }
...
func (c readFramebufferDepth) Replay(id atom.ID, s *state.State, b *builder.Builder, wantOutput bool) {
	fbW, fbH, err := s.Contexts[c.contextID].GetFramebufferAttachmentSize(state.FramebufferAttachmentDepth)
	if err != nil {
		b.EndAtom()
		return
	}
...
func (c readFramebufferColor) Replay(id atom.ID, s *state.State, b *builder.Builder, wantOutput bool) {
	imageSize := uint64(c.height * c.width * 4)
	addr := b.AllocateTemporaryMemory(imageSize)

	fbW, fbH, err := s.Contexts[c.contextID].GetFramebufferAttachmentSize(state.FramebufferAttachmentColor)
	if err != nil {
		b.EndAtom()
		return
	}
