////////////////////////////////////////////////////////////////////////////////
// Do not modify!
// Generated by codergen -go
////////////////////////////////////////////////////////////////////////////////

package test

import (
	"android.googlesource.com/platform/tools/gpu/atom"
	"android.googlesource.com/platform/tools/gpu/binary"
	"android.googlesource.com/platform/tools/gpu/memory"
)

func init() {
	//struct test.CmdArrayOfFloat { Context:atom.ContextID, In:CmdArrayOfFloat_In, Out:CmdArrayOfFloat_Out }
	binary.Register(binary.ID{0xc7, 0xbe, 0x8b, 0xe6, 0x15, 0x6e, 0x20, 0x18, 0x6c, 0xec, 0xb3, 0xdf, 0xa3, 0x66, 0xae, 0xf0, 0x94, 0xe0, 0x48, 0xda}, &CmdArrayOfFloat{})
	//struct test.CmdArrayOfFloat_In { }
	binary.Register(binary.ID{0xd5, 0x2b, 0xc7, 0x41, 0xb0, 0x20, 0x45, 0x1e, 0x83, 0xbd, 0x77, 0xf4, 0xca, 0x4d, 0x9a, 0x2c, 0xbb, 0x2b, 0x07, 0x90}, &CmdArrayOfFloat_In{})
	//struct test.CmdArrayOfFloat_Out { Result:F32Array }
	binary.Register(binary.ID{0x36, 0x3c, 0x3e, 0x10, 0x7a, 0xbb, 0xa4, 0x7c, 0x75, 0x9a, 0x9f, 0x19, 0x07, 0x1b, 0x7e, 0x9a, 0x95, 0x0d, 0x1c, 0x30}, &CmdArrayOfFloat_Out{})
	//struct test.CmdBool { Context:atom.ContextID, In:CmdBool_In, Out:CmdBool_Out }
	binary.Register(binary.ID{0xfc, 0x9c, 0x18, 0xaa, 0x58, 0xf9, 0x5c, 0x18, 0xfe, 0x28, 0xc6, 0x72, 0x0a, 0xa7, 0x2e, 0xb9, 0xcc, 0x7d, 0xde, 0x21}, &CmdBool{})
	//struct test.CmdBool_In { }
	binary.Register(binary.ID{0xa8, 0xc3, 0xf0, 0xd7, 0xdf, 0x03, 0x75, 0x19, 0x03, 0x76, 0x72, 0x4c, 0xb8, 0xd4, 0x55, 0x0a, 0xff, 0x6f, 0x46, 0xcf}, &CmdBool_In{})
	//struct test.CmdBool_Out { Result:bool }
	binary.Register(binary.ID{0x6f, 0x9e, 0x3b, 0xfa, 0xe7, 0xd8, 0x25, 0x76, 0xe7, 0x91, 0x65, 0x6c, 0x99, 0xb3, 0x80, 0x04, 0x17, 0xf4, 0xb8, 0xd8}, &CmdBool_Out{})
	//struct test.CmdF32 { Context:atom.ContextID, In:CmdF32_In, Out:CmdF32_Out }
	binary.Register(binary.ID{0xda, 0xaf, 0xbd, 0xf3, 0xcb, 0x30, 0xb6, 0x68, 0x7a, 0xcd, 0xf4, 0xc8, 0x54, 0x6b, 0x47, 0x6d, 0x29, 0x03, 0xe5, 0xca}, &CmdF32{})
	//struct test.CmdF32_In { }
	binary.Register(binary.ID{0x37, 0x2e, 0x12, 0xe8, 0xb9, 0xf9, 0xdf, 0x55, 0x73, 0x54, 0x0c, 0xb7, 0x0a, 0xcf, 0xb6, 0x3a, 0x5d, 0x8e, 0xc0, 0xea}, &CmdF32_In{})
	//struct test.CmdF32_Out { Result:float32 }
	binary.Register(binary.ID{0xa5, 0x94, 0x0e, 0xe8, 0x83, 0xf7, 0xb9, 0x64, 0x34, 0x1a, 0x70, 0xf4, 0x75, 0x8a, 0x61, 0xf7, 0x62, 0x8b, 0xe5, 0x7b}, &CmdF32_Out{})
	//struct test.CmdF64 { Context:atom.ContextID, In:CmdF64_In, Out:CmdF64_Out }
	binary.Register(binary.ID{0x42, 0x3c, 0x1a, 0x84, 0x64, 0x17, 0xaf, 0xe4, 0x29, 0x6f, 0x9e, 0x59, 0x95, 0xd6, 0x38, 0xd5, 0x83, 0xd8, 0x67, 0x75}, &CmdF64{})
	//struct test.CmdF64_In { }
	binary.Register(binary.ID{0x52, 0xb1, 0x85, 0xd0, 0x4b, 0xb3, 0x41, 0xf5, 0x38, 0xd4, 0xc2, 0xfd, 0x72, 0x2d, 0x11, 0x9b, 0x75, 0xb0, 0x89, 0xea}, &CmdF64_In{})
	//struct test.CmdF64_Out { Result:float64 }
	binary.Register(binary.ID{0x54, 0x76, 0x58, 0x05, 0x51, 0xe0, 0x1e, 0x09, 0xc5, 0x2a, 0x9c, 0xdc, 0xfa, 0xb5, 0x96, 0x0a, 0xd2, 0x18, 0xef, 0x66}, &CmdF64_Out{})
	//struct test.CmdPointer { Context:atom.ContextID, In:CmdPointer_In, Out:CmdPointer_Out }
	binary.Register(binary.ID{0xbf, 0x67, 0x3e, 0x0f, 0x67, 0x62, 0x75, 0x98, 0x1a, 0xc2, 0x36, 0x67, 0xec, 0x14, 0x95, 0x83, 0x6f, 0x96, 0xfb, 0x86}, &CmdPointer{})
	//struct test.CmdPointer_In { }
	binary.Register(binary.ID{0x02, 0x87, 0x43, 0x42, 0xe1, 0xcf, 0x50, 0x41, 0x05, 0x53, 0x0d, 0x1c, 0xa7, 0x77, 0x4a, 0xfe, 0xf8, 0x99, 0x68, 0x27}, &CmdPointer_In{})
	//struct test.CmdPointer_Out { Result:memory.Pointer }
	binary.Register(binary.ID{0xe3, 0xa5, 0xa0, 0xc2, 0x07, 0xfc, 0xf6, 0xbd, 0x8d, 0x89, 0xae, 0xb2, 0x02, 0xdf, 0xeb, 0x15, 0x0a, 0xf0, 0x65, 0x12}, &CmdPointer_Out{})
	//struct test.CmdS16 { Context:atom.ContextID, In:CmdS16_In, Out:CmdS16_Out }
	binary.Register(binary.ID{0x7c, 0x7c, 0x2d, 0x85, 0x27, 0x81, 0xd9, 0xc7, 0xf2, 0x53, 0x3b, 0x0b, 0x81, 0x13, 0xc8, 0xbc, 0xd4, 0x23, 0xe9, 0x1b}, &CmdS16{})
	//struct test.CmdS16_In { }
	binary.Register(binary.ID{0xb4, 0x21, 0xaf, 0x09, 0xe1, 0xcb, 0x81, 0x1f, 0x25, 0x29, 0xaf, 0x09, 0x9e, 0xf5, 0x48, 0x32, 0xb2, 0x99, 0xfd, 0x74}, &CmdS16_In{})
	//struct test.CmdS16_Out { Result:int16 }
	binary.Register(binary.ID{0x6f, 0x2b, 0xe4, 0x31, 0xff, 0x53, 0xb9, 0x35, 0xc9, 0x13, 0x54, 0x06, 0xdb, 0x9c, 0x04, 0xcd, 0x69, 0x27, 0xa9, 0x74}, &CmdS16_Out{})
	//struct test.CmdS32 { Context:atom.ContextID, In:CmdS32_In, Out:CmdS32_Out }
	binary.Register(binary.ID{0x63, 0xf9, 0x22, 0x35, 0x17, 0xe0, 0x49, 0xc0, 0x77, 0x8f, 0x23, 0x22, 0x4f, 0x84, 0x9d, 0xc9, 0x50, 0xea, 0x10, 0x02}, &CmdS32{})
	//struct test.CmdS32_In { }
	binary.Register(binary.ID{0xa2, 0x0c, 0x1a, 0x78, 0x13, 0x5b, 0xb9, 0xf4, 0xb9, 0x84, 0x46, 0x26, 0x3c, 0xbc, 0xbc, 0x6c, 0x28, 0x96, 0xa6, 0x2d}, &CmdS32_In{})
	//struct test.CmdS32_Out { Result:int32 }
	binary.Register(binary.ID{0x83, 0x99, 0xd5, 0xe8, 0xb6, 0xc5, 0x6a, 0x3b, 0xf5, 0x20, 0x22, 0x59, 0x40, 0xc9, 0xde, 0x88, 0x09, 0x09, 0xa5, 0xe9}, &CmdS32_Out{})
	//struct test.CmdS64 { Context:atom.ContextID, In:CmdS64_In, Out:CmdS64_Out }
	binary.Register(binary.ID{0xbd, 0x2e, 0x78, 0xa7, 0x9a, 0x51, 0xad, 0xc9, 0xfa, 0x1b, 0x28, 0x52, 0xb8, 0xc8, 0xcc, 0x63, 0xc1, 0xec, 0xae, 0xea}, &CmdS64{})
	//struct test.CmdS64_In { }
	binary.Register(binary.ID{0x69, 0x7f, 0x8a, 0x03, 0xf3, 0xd1, 0x13, 0x1b, 0x1c, 0x15, 0xc8, 0x9f, 0x23, 0x73, 0x1c, 0xc2, 0xfd, 0x9e, 0xe3, 0x3b}, &CmdS64_In{})
	//struct test.CmdS64_Out { Result:int64 }
	binary.Register(binary.ID{0xae, 0x38, 0x61, 0x6c, 0x5c, 0xb0, 0x30, 0xcc, 0xce, 0xce, 0x4e, 0xf0, 0x9d, 0x70, 0x0f, 0xd2, 0x4c, 0x11, 0x68, 0x86}, &CmdS64_Out{})
	//struct test.CmdS8 { Context:atom.ContextID, In:CmdS8_In, Out:CmdS8_Out }
	binary.Register(binary.ID{0x29, 0x72, 0x6a, 0x87, 0xc4, 0xe1, 0x2f, 0x7c, 0x63, 0x50, 0xa4, 0xc7, 0x74, 0xde, 0xf1, 0xf4, 0xe0, 0x5e, 0x05, 0x4c}, &CmdS8{})
	//struct test.CmdS8_In { }
	binary.Register(binary.ID{0xc7, 0xb6, 0xa0, 0x8b, 0xfb, 0xc6, 0x57, 0x3d, 0x90, 0x4d, 0x6b, 0x67, 0xfc, 0x5a, 0x3d, 0x59, 0x87, 0x3d, 0x8e, 0xb1}, &CmdS8_In{})
	//struct test.CmdS8_Out { Result:int8 }
	binary.Register(binary.ID{0xf9, 0xf1, 0x54, 0x03, 0x51, 0xfc, 0xd1, 0xe0, 0xf2, 0x74, 0x68, 0xb8, 0x9a, 0xf5, 0x53, 0x46, 0xd1, 0xb3, 0xf4, 0x7b}, &CmdS8_Out{})
	//struct test.CmdString { Context:atom.ContextID, In:CmdString_In, Out:CmdString_Out }
	binary.Register(binary.ID{0xb7, 0x58, 0xe4, 0xf9, 0xd0, 0x2e, 0x81, 0x4f, 0x94, 0x2d, 0xc9, 0x95, 0x86, 0xcf, 0x31, 0xc0, 0x86, 0xe3, 0x2d, 0x03}, &CmdString{})
	//struct test.CmdString_In { }
	binary.Register(binary.ID{0x00, 0xe3, 0x87, 0xcd, 0xd3, 0xd5, 0x7d, 0x83, 0x8f, 0x1d, 0xf9, 0x26, 0x3d, 0x5a, 0x9c, 0x87, 0xb8, 0x31, 0x8c, 0x70}, &CmdString_In{})
	//struct test.CmdString_Out { Result:string }
	binary.Register(binary.ID{0x49, 0x91, 0x68, 0x46, 0x4b, 0x1e, 0xf5, 0x69, 0x9f, 0x35, 0x4b, 0x6f, 0x13, 0x6a, 0xbe, 0x56, 0x49, 0x3a, 0xd9, 0x23}, &CmdString_Out{})
	//struct test.CmdU16 { Context:atom.ContextID, In:CmdU16_In, Out:CmdU16_Out }
	binary.Register(binary.ID{0x7d, 0xc6, 0xc1, 0x51, 0xe1, 0xbf, 0x87, 0x67, 0x33, 0x28, 0xf8, 0xbc, 0x93, 0xaa, 0xe8, 0xb9, 0xad, 0x5c, 0x65, 0xfc}, &CmdU16{})
	//struct test.CmdU16_In { }
	binary.Register(binary.ID{0xd2, 0xa1, 0x65, 0xb8, 0x84, 0x2d, 0x34, 0x0a, 0x3a, 0x73, 0x08, 0x34, 0xfb, 0xa6, 0x44, 0xf2, 0x1c, 0x36, 0x3a, 0xdc}, &CmdU16_In{})
	//struct test.CmdU16_Out { Result:uint16 }
	binary.Register(binary.ID{0x80, 0x9c, 0x36, 0x68, 0x0d, 0xff, 0x0c, 0x69, 0xb4, 0x59, 0x87, 0x55, 0x87, 0x78, 0x36, 0x99, 0x7f, 0x8d, 0x75, 0x98}, &CmdU16_Out{})
	//struct test.CmdU32 { Context:atom.ContextID, In:CmdU32_In, Out:CmdU32_Out }
	binary.Register(binary.ID{0x41, 0x36, 0xcb, 0x68, 0xb9, 0x2b, 0xc4, 0x61, 0xfa, 0x26, 0x03, 0x81, 0xbd, 0x3d, 0xbe, 0x7b, 0x04, 0x3c, 0x98, 0x5b}, &CmdU32{})
	//struct test.CmdU32_In { }
	binary.Register(binary.ID{0x73, 0x20, 0x0d, 0x68, 0x76, 0x5b, 0x5a, 0xae, 0x0a, 0xf1, 0xef, 0x70, 0x50, 0x0d, 0x66, 0xe8, 0xe9, 0xbe, 0x79, 0xf8}, &CmdU32_In{})
	//struct test.CmdU32_Out { Result:uint32 }
	binary.Register(binary.ID{0x36, 0x73, 0x3e, 0x1d, 0x85, 0x32, 0x08, 0x71, 0xba, 0x61, 0xbe, 0xaa, 0x42, 0xf0, 0x3a, 0x0a, 0x33, 0x1d, 0x00, 0x80}, &CmdU32_Out{})
	//struct test.CmdU64 { Context:atom.ContextID, In:CmdU64_In, Out:CmdU64_Out }
	binary.Register(binary.ID{0xcf, 0xd9, 0xb9, 0x9c, 0x49, 0xcf, 0xeb, 0xbf, 0x69, 0x3d, 0x83, 0xfa, 0x61, 0x98, 0x84, 0xe0, 0xa4, 0xa7, 0x3f, 0xeb}, &CmdU64{})
	//struct test.CmdU64_In { }
	binary.Register(binary.ID{0xf9, 0x54, 0x3e, 0x9a, 0xe2, 0x2a, 0x29, 0x49, 0xec, 0x6d, 0x3c, 0x5a, 0xe5, 0xaf, 0x0d, 0x82, 0xb2, 0x40, 0xad, 0xe1}, &CmdU64_In{})
	//struct test.CmdU64_Out { Result:uint64 }
	binary.Register(binary.ID{0xc7, 0x94, 0x16, 0x55, 0x11, 0x63, 0x43, 0xe8, 0x65, 0x97, 0x2f, 0x34, 0x5f, 0x60, 0x43, 0xe9, 0xa8, 0x5e, 0x5a, 0x42}, &CmdU64_Out{})
	//struct test.CmdU8 { Context:atom.ContextID, In:CmdU8_In, Out:CmdU8_Out }
	binary.Register(binary.ID{0x03, 0x49, 0x07, 0x3a, 0xb3, 0xdf, 0xb8, 0xcf, 0x43, 0xae, 0x0b, 0x22, 0xfb, 0x5d, 0xf3, 0x01, 0x01, 0x7e, 0xa9, 0x68}, &CmdU8{})
	//struct test.CmdU8_In { }
	binary.Register(binary.ID{0xf4, 0x59, 0x68, 0x38, 0xe0, 0xa8, 0xec, 0x9e, 0xb2, 0x95, 0x67, 0xe4, 0x5e, 0xca, 0x90, 0x7b, 0xeb, 0xc0, 0xac, 0x98}, &CmdU8_In{})
	//struct test.CmdU8_Out { Result:uint8 }
	binary.Register(binary.ID{0xce, 0x2e, 0x2a, 0xec, 0xd3, 0xeb, 0x97, 0x56, 0xfb, 0x73, 0x1d, 0x83, 0xb1, 0x73, 0x68, 0xa5, 0x4b, 0xbf, 0x36, 0xf2}, &CmdU8_Out{})
	//struct test.CmdVoid { Context:atom.ContextID, In:CmdVoid_In, Out:CmdVoid_Out }
	binary.Register(binary.ID{0x88, 0x4a, 0x99, 0xf4, 0xfa, 0xe0, 0xc3, 0xfa, 0x45, 0xdf, 0x5a, 0xf0, 0xd9, 0x15, 0x1a, 0x30, 0xb0, 0x04, 0xb4, 0xd8}, &CmdVoid{})
	//struct test.CmdVoid3Arrays { Context:atom.ContextID, In:CmdVoid3Arrays_In, Out:CmdVoid3Arrays_Out }
	binary.Register(binary.ID{0x34, 0xd8, 0x53, 0x48, 0x0c, 0xa9, 0x39, 0xf1, 0xee, 0x48, 0xec, 0x05, 0x50, 0x75, 0x9d, 0x9d, 0xf7, 0xca, 0x1d, 0x07}, &CmdVoid3Arrays{})
	//struct test.CmdVoid3Arrays_In { A:S8Array, B:StringArray, C:BoolArray }
	binary.Register(binary.ID{0x9b, 0x77, 0x88, 0x62, 0x7c, 0x3f, 0xd7, 0x49, 0xaa, 0x32, 0xff, 0x17, 0xd1, 0x65, 0xcc, 0xda, 0xaf, 0xad, 0x8e, 0xf0}, &CmdVoid3Arrays_In{})
	//struct test.CmdVoid3Arrays_Out { }
	binary.Register(binary.ID{0xfe, 0x87, 0x9c, 0x49, 0x76, 0x14, 0x11, 0xd1, 0x60, 0xa2, 0x61, 0xee, 0xbf, 0x09, 0x43, 0x0e, 0x45, 0x08, 0x97, 0x52}, &CmdVoid3Arrays_Out{})
	//struct test.CmdVoid3Remapped { Context:atom.ContextID, In:CmdVoid3Remapped_In, Out:CmdVoid3Remapped_Out }
	binary.Register(binary.ID{0xe4, 0x31, 0xc2, 0x20, 0x2b, 0x84, 0x05, 0xd6, 0x48, 0xa9, 0x2d, 0xd7, 0xff, 0x4e, 0xce, 0xf1, 0xf8, 0xe9, 0x4e, 0xbd}, &CmdVoid3Remapped{})
	//struct test.CmdVoid3Remapped_In { A:remapped, B:remapped, C:remapped }
	binary.Register(binary.ID{0x65, 0x9c, 0xe6, 0x1c, 0x4c, 0x6e, 0xe5, 0xed, 0x14, 0xcb, 0xe9, 0x57, 0xee, 0x05, 0xc6, 0x43, 0x4e, 0x7e, 0x45, 0x42}, &CmdVoid3Remapped_In{})
	//struct test.CmdVoid3Remapped_Out { }
	binary.Register(binary.ID{0x60, 0x0a, 0xfe, 0x1a, 0xa7, 0xb0, 0x3c, 0x64, 0xa2, 0x73, 0x7a, 0xa9, 0xff, 0x89, 0xe8, 0xc8, 0x18, 0x3a, 0xca, 0xd0}, &CmdVoid3Remapped_Out{})
	//struct test.CmdVoid3Strings { Context:atom.ContextID, In:CmdVoid3Strings_In, Out:CmdVoid3Strings_Out }
	binary.Register(binary.ID{0x42, 0xa0, 0xbc, 0xde, 0x7d, 0x62, 0x67, 0x5e, 0xe3, 0x8f, 0xd3, 0x76, 0x92, 0x8b, 0xa0, 0x9e, 0xfe, 0x6f, 0x1c, 0xf8}, &CmdVoid3Strings{})
	//struct test.CmdVoid3Strings_In { A:string, B:string, C:string }
	binary.Register(binary.ID{0x85, 0x28, 0x3e, 0x66, 0x72, 0xcc, 0x8a, 0xd9, 0x1a, 0xac, 0x60, 0x7c, 0xb7, 0x54, 0x99, 0x58, 0x9e, 0x7c, 0x50, 0x2f}, &CmdVoid3Strings_In{})
	//struct test.CmdVoid3Strings_Out { }
	binary.Register(binary.ID{0x4c, 0x9e, 0x35, 0x94, 0xa5, 0x95, 0x17, 0xd6, 0x6c, 0x33, 0x21, 0xf1, 0xfc, 0xf2, 0x18, 0x89, 0x36, 0x9a, 0x48, 0x30}, &CmdVoid3Strings_Out{})
	//struct test.CmdVoidArrayOfStrings { Context:atom.ContextID, In:CmdVoidArrayOfStrings_In, Out:CmdVoidArrayOfStrings_Out }
	binary.Register(binary.ID{0xf7, 0xb9, 0x87, 0x78, 0x8b, 0x97, 0x71, 0x36, 0x8e, 0x42, 0x4f, 0x47, 0xf6, 0xbb, 0x1c, 0x80, 0x4f, 0xd2, 0x1b, 0x82}, &CmdVoidArrayOfStrings{})
	//struct test.CmdVoidArrayOfStrings_In { A:StringArray }
	binary.Register(binary.ID{0x86, 0xc8, 0x7e, 0x7c, 0x94, 0x4c, 0x70, 0xba, 0x02, 0x8a, 0x73, 0xf2, 0x63, 0x93, 0x29, 0x2d, 0x7c, 0xc5, 0x17, 0x28}, &CmdVoidArrayOfStrings_In{})
	//struct test.CmdVoidArrayOfStrings_Out { }
	binary.Register(binary.ID{0x42, 0x2a, 0xaf, 0xb7, 0x87, 0x49, 0xd6, 0x31, 0xed, 0xd7, 0xfa, 0xd7, 0xa7, 0x94, 0x3b, 0xbd, 0x4c, 0xfd, 0x83, 0x3c}, &CmdVoidArrayOfStrings_Out{})
	//struct test.CmdVoidBool { Context:atom.ContextID, In:CmdVoidBool_In, Out:CmdVoidBool_Out }
	binary.Register(binary.ID{0xc1, 0x9c, 0xeb, 0xb3, 0xfb, 0xec, 0xc6, 0xb9, 0x90, 0x04, 0x99, 0xdb, 0x0a, 0x33, 0x5f, 0xda, 0x12, 0x38, 0xfa, 0x06}, &CmdVoidBool{})
	//struct test.CmdVoidBool_In { A:bool }
	binary.Register(binary.ID{0x15, 0xed, 0xa2, 0xcd, 0xec, 0x73, 0xbc, 0x31, 0x1c, 0xc7, 0xaa, 0x20, 0x1e, 0x50, 0xb8, 0x30, 0x12, 0xf2, 0xea, 0xab}, &CmdVoidBool_In{})
	//struct test.CmdVoidBool_Out { }
	binary.Register(binary.ID{0x08, 0x1e, 0xf8, 0xd5, 0xb6, 0x95, 0x1b, 0x66, 0x97, 0xfd, 0x06, 0xaf, 0x76, 0x3b, 0xa4, 0xfd, 0x09, 0xf6, 0x0c, 0xf7}, &CmdVoidBool_Out{})
	//struct test.CmdVoidF32 { Context:atom.ContextID, In:CmdVoidF32_In, Out:CmdVoidF32_Out }
	binary.Register(binary.ID{0x60, 0x5d, 0x8c, 0x59, 0xc3, 0xea, 0x56, 0xcf, 0x11, 0x68, 0x04, 0xd5, 0xe5, 0x31, 0xce, 0x2e, 0xd3, 0x32, 0x22, 0xe2}, &CmdVoidF32{})
	//struct test.CmdVoidF32_In { A:float32 }
	binary.Register(binary.ID{0x7f, 0x8a, 0x76, 0x16, 0x25, 0xe5, 0x4f, 0x6e, 0x88, 0xdc, 0xd4, 0x97, 0x04, 0x8c, 0x22, 0x75, 0xc9, 0xc6, 0xa1, 0x3c}, &CmdVoidF32_In{})
	//struct test.CmdVoidF32_Out { }
	binary.Register(binary.ID{0xb8, 0xe3, 0x7c, 0x1a, 0xa9, 0x57, 0x2b, 0x62, 0x49, 0xcc, 0xeb, 0x03, 0xa8, 0xd4, 0xf7, 0x81, 0x29, 0x61, 0x2d, 0x47}, &CmdVoidF32_Out{})
	//struct test.CmdVoidF64 { Context:atom.ContextID, In:CmdVoidF64_In, Out:CmdVoidF64_Out }
	binary.Register(binary.ID{0xac, 0x85, 0xda, 0xe0, 0x41, 0x9d, 0xf8, 0x2d, 0x50, 0x9d, 0xae, 0x08, 0x2c, 0x0c, 0x7b, 0xb0, 0x30, 0x75, 0xcd, 0xb4}, &CmdVoidF64{})
	//struct test.CmdVoidF64_In { A:float64 }
	binary.Register(binary.ID{0xed, 0xf7, 0xff, 0xb1, 0x65, 0xdf, 0x39, 0x7e, 0x22, 0xa7, 0x60, 0x10, 0x44, 0xfe, 0x95, 0x6a, 0x51, 0x1d, 0x04, 0xa2}, &CmdVoidF64_In{})
	//struct test.CmdVoidF64_Out { }
	binary.Register(binary.ID{0xc1, 0xe2, 0x8d, 0xd9, 0xa0, 0xf2, 0xd6, 0x31, 0x3f, 0xcf, 0x6b, 0x33, 0x5c, 0x88, 0x54, 0xb9, 0xe0, 0x16, 0x0d, 0x18}, &CmdVoidF64_Out{})
	//struct test.CmdVoidOut3Remapped { Context:atom.ContextID, In:CmdVoidOut3Remapped_In, Out:CmdVoidOut3Remapped_Out }
	binary.Register(binary.ID{0xd5, 0x42, 0x81, 0x40, 0x1c, 0x68, 0x52, 0x47, 0xca, 0x63, 0x02, 0x1c, 0x53, 0xda, 0xae, 0xc3, 0x16, 0xf4, 0xf8, 0x75}, &CmdVoidOut3Remapped{})
	//struct test.CmdVoidOut3Remapped_In { }
	binary.Register(binary.ID{0x5c, 0x6b, 0x8f, 0xaf, 0x40, 0xc0, 0x25, 0x1d, 0x27, 0x99, 0x7b, 0xd9, 0x5f, 0xba, 0x3e, 0x20, 0xa2, 0x74, 0x8e, 0x3c}, &CmdVoidOut3Remapped_In{})
	//struct test.CmdVoidOut3Remapped_Out { A:remapped, B:remapped, C:remapped }
	binary.Register(binary.ID{0xb6, 0x69, 0x41, 0xc0, 0x8a, 0x84, 0x88, 0xc3, 0x3f, 0x94, 0xcc, 0x3a, 0xc1, 0x02, 0x48, 0x92, 0x2b, 0x53, 0xbc, 0xea}, &CmdVoidOut3Remapped_Out{})
	//struct test.CmdVoidOut3Strings { Context:atom.ContextID, In:CmdVoidOut3Strings_In, Out:CmdVoidOut3Strings_Out }
	binary.Register(binary.ID{0x56, 0x9b, 0x78, 0xc4, 0x56, 0xce, 0xfb, 0x66, 0x40, 0x19, 0xb6, 0xe5, 0xf9, 0x1e, 0x76, 0x49, 0x8a, 0x8b, 0xfd, 0xad}, &CmdVoidOut3Strings{})
	//struct test.CmdVoidOut3Strings_In { }
	binary.Register(binary.ID{0x02, 0xb0, 0x9c, 0x20, 0xaf, 0xeb, 0x29, 0x43, 0xe1, 0x4d, 0x06, 0x2f, 0xe1, 0x8a, 0x12, 0x04, 0x98, 0x9c, 0xe5, 0xcf}, &CmdVoidOut3Strings_In{})
	//struct test.CmdVoidOut3Strings_Out { A:string, B:string, C:string }
	binary.Register(binary.ID{0xcf, 0xd1, 0x6b, 0xe1, 0xc6, 0xc1, 0xe5, 0x3d, 0xee, 0x9c, 0xd6, 0x3d, 0x86, 0xfb, 0x95, 0xff, 0xee, 0x10, 0x78, 0xcf}, &CmdVoidOut3Strings_Out{})
	//struct test.CmdVoidOutArrayOfRemapped { Context:atom.ContextID, In:CmdVoidOutArrayOfRemapped_In, Out:CmdVoidOutArrayOfRemapped_Out }
	binary.Register(binary.ID{0xc8, 0x7c, 0x03, 0x3f, 0x92, 0x37, 0xd8, 0xcd, 0x75, 0x11, 0xab, 0xee, 0xc9, 0xcc, 0xde, 0xd6, 0x4c, 0x7f, 0x9e, 0x5e}, &CmdVoidOutArrayOfRemapped{})
	//struct test.CmdVoidOutArrayOfRemapped_In { }
	binary.Register(binary.ID{0x8e, 0xf9, 0x7a, 0xc5, 0x69, 0x1f, 0xc5, 0x94, 0x17, 0x22, 0xc7, 0xd3, 0x4a, 0xa4, 0xe1, 0x2a, 0x2e, 0x01, 0xac, 0x88}, &CmdVoidOutArrayOfRemapped_In{})
	//struct test.CmdVoidOutArrayOfRemapped_Out { A:RemappedArray }
	binary.Register(binary.ID{0x9e, 0x88, 0x02, 0x47, 0x74, 0xbc, 0x64, 0xdf, 0xbc, 0x37, 0x15, 0xb3, 0xb4, 0x38, 0x9b, 0xab, 0x21, 0x81, 0x02, 0x90}, &CmdVoidOutArrayOfRemapped_Out{})
	//struct test.CmdVoidOutBool { Context:atom.ContextID, In:CmdVoidOutBool_In, Out:CmdVoidOutBool_Out }
	binary.Register(binary.ID{0x1c, 0xcb, 0x3c, 0x37, 0x26, 0xf5, 0xb0, 0x6d, 0x66, 0x3e, 0xf5, 0x83, 0x48, 0x13, 0x50, 0xfe, 0x95, 0x5b, 0x8e, 0x20}, &CmdVoidOutBool{})
	//struct test.CmdVoidOutBool_In { }
	binary.Register(binary.ID{0x80, 0x53, 0x0f, 0x2e, 0x35, 0x68, 0x1f, 0x68, 0x12, 0x68, 0x0b, 0xb6, 0x47, 0xf2, 0x93, 0xcb, 0xe2, 0xa5, 0x01, 0x95}, &CmdVoidOutBool_In{})
	//struct test.CmdVoidOutBool_Out { A:bool }
	binary.Register(binary.ID{0x22, 0xa8, 0xe6, 0x66, 0x3f, 0x29, 0xba, 0xfd, 0x51, 0xec, 0x33, 0xb4, 0xed, 0xcd, 0xd3, 0xd5, 0xbe, 0x83, 0xd2, 0x8e}, &CmdVoidOutBool_Out{})
	//struct test.CmdVoidOutF32 { Context:atom.ContextID, In:CmdVoidOutF32_In, Out:CmdVoidOutF32_Out }
	binary.Register(binary.ID{0x1d, 0x41, 0xce, 0xd7, 0x6a, 0x15, 0x8d, 0xab, 0xe0, 0x8e, 0x47, 0x0b, 0x4f, 0x3e, 0x9b, 0x1c, 0x95, 0x98, 0x23, 0xee}, &CmdVoidOutF32{})
	//struct test.CmdVoidOutF32_In { }
	binary.Register(binary.ID{0x9c, 0x56, 0x82, 0x30, 0xbb, 0x8b, 0x12, 0x84, 0x6a, 0xf2, 0x47, 0x8e, 0x4d, 0xcd, 0xfc, 0x43, 0xf1, 0x0c, 0x68, 0xa5}, &CmdVoidOutF32_In{})
	//struct test.CmdVoidOutF32_Out { A:float32 }
	binary.Register(binary.ID{0x61, 0x33, 0xd0, 0xe0, 0xab, 0x9d, 0x49, 0x7a, 0xbd, 0x54, 0x63, 0x31, 0x94, 0x31, 0xbf, 0x72, 0xf5, 0x5c, 0x04, 0x2c}, &CmdVoidOutF32_Out{})
	//struct test.CmdVoidOutF64 { Context:atom.ContextID, In:CmdVoidOutF64_In, Out:CmdVoidOutF64_Out }
	binary.Register(binary.ID{0x91, 0x80, 0x24, 0x86, 0x48, 0xe3, 0x08, 0x0c, 0x45, 0xfe, 0xa1, 0x19, 0x26, 0xdb, 0xc2, 0x46, 0xc7, 0x8d, 0xc2, 0x35}, &CmdVoidOutF64{})
	//struct test.CmdVoidOutF64_In { }
	binary.Register(binary.ID{0x71, 0x03, 0x21, 0x58, 0x82, 0x18, 0xe3, 0x75, 0xf7, 0xe5, 0xf1, 0xc4, 0x9e, 0x43, 0xe3, 0x97, 0x14, 0x57, 0x33, 0xde}, &CmdVoidOutF64_In{})
	//struct test.CmdVoidOutF64_Out { A:float64 }
	binary.Register(binary.ID{0x74, 0x18, 0x51, 0x17, 0x14, 0x72, 0x02, 0x47, 0x94, 0x32, 0xa8, 0xe6, 0xc4, 0x12, 0xdc, 0x80, 0xe4, 0x00, 0x1c, 0x6f}, &CmdVoidOutF64_Out{})
	//struct test.CmdVoidOutFixedSizeBuffer { Context:atom.ContextID, In:CmdVoidOutFixedSizeBuffer_In, Out:CmdVoidOutFixedSizeBuffer_Out }
	binary.Register(binary.ID{0x21, 0x7a, 0x15, 0x75, 0xc1, 0x28, 0x61, 0x86, 0x5b, 0xd7, 0x7a, 0xf7, 0x51, 0xef, 0xb5, 0xdf, 0x26, 0x9e, 0xf7, 0x6d}, &CmdVoidOutFixedSizeBuffer{})
	//struct test.CmdVoidOutFixedSizeBuffer_In { }
	binary.Register(binary.ID{0x7b, 0x3d, 0x39, 0xa6, 0x2f, 0x5d, 0xa4, 0xe3, 0x9c, 0x88, 0xe4, 0x23, 0x09, 0x67, 0x4f, 0x5e, 0xec, 0xaf, 0xb5, 0xe6}, &CmdVoidOutFixedSizeBuffer_In{})
	//struct test.CmdVoidOutFixedSizeBuffer_Out { A:memory.Pointer }
	binary.Register(binary.ID{0x74, 0xf3, 0x71, 0x06, 0x6d, 0xbd, 0x2f, 0x37, 0xde, 0xfc, 0x57, 0xfd, 0x6e, 0xd2, 0xf5, 0x9e, 0x88, 0x34, 0x07, 0xed}, &CmdVoidOutFixedSizeBuffer_Out{})
	//struct test.CmdVoidOutS16 { Context:atom.ContextID, In:CmdVoidOutS16_In, Out:CmdVoidOutS16_Out }
	binary.Register(binary.ID{0xd2, 0x6e, 0xd4, 0xc6, 0xf1, 0xaf, 0x77, 0x18, 0xab, 0xb0, 0xd2, 0x2c, 0xd8, 0xe0, 0x7e, 0x1f, 0x63, 0xed, 0xa0, 0xf4}, &CmdVoidOutS16{})
	//struct test.CmdVoidOutS16_In { }
	binary.Register(binary.ID{0xde, 0x5b, 0x1b, 0x9d, 0xd9, 0xd9, 0xf5, 0xbd, 0xc7, 0x50, 0x1c, 0x24, 0x7a, 0xb1, 0xec, 0x3b, 0x3c, 0xcc, 0xb1, 0xa6}, &CmdVoidOutS16_In{})
	//struct test.CmdVoidOutS16_Out { A:int16 }
	binary.Register(binary.ID{0xd4, 0x93, 0xe1, 0x5e, 0xcf, 0x55, 0xf5, 0x5b, 0x2c, 0x0f, 0xc5, 0x50, 0x5f, 0x63, 0x10, 0xe0, 0x89, 0x1a, 0x4a, 0xb2}, &CmdVoidOutS16_Out{})
	//struct test.CmdVoidOutS32 { Context:atom.ContextID, In:CmdVoidOutS32_In, Out:CmdVoidOutS32_Out }
	binary.Register(binary.ID{0xcc, 0x0e, 0x27, 0xd7, 0x51, 0xc7, 0xf5, 0x76, 0xde, 0x4a, 0xb1, 0x6a, 0xce, 0x9e, 0xd4, 0xe4, 0x75, 0xb4, 0x23, 0x5d}, &CmdVoidOutS32{})
	//struct test.CmdVoidOutS32_In { }
	binary.Register(binary.ID{0x59, 0xd6, 0x69, 0xb5, 0xd9, 0xdd, 0x1e, 0x51, 0x2d, 0xce, 0xf0, 0x7a, 0x76, 0x32, 0xd4, 0xdf, 0xf4, 0x3a, 0x77, 0x69}, &CmdVoidOutS32_In{})
	//struct test.CmdVoidOutS32_Out { A:int32 }
	binary.Register(binary.ID{0xac, 0x2c, 0xba, 0x2f, 0x42, 0x97, 0x82, 0xc5, 0x8d, 0x6d, 0xa0, 0xee, 0x81, 0xb0, 0x41, 0x8f, 0xc4, 0xfb, 0xd1, 0x14}, &CmdVoidOutS32_Out{})
	//struct test.CmdVoidOutS64 { Context:atom.ContextID, In:CmdVoidOutS64_In, Out:CmdVoidOutS64_Out }
	binary.Register(binary.ID{0xd8, 0x2d, 0xf3, 0x72, 0x09, 0xb0, 0x12, 0x11, 0xaf, 0x75, 0x0a, 0x5c, 0x73, 0x4f, 0xce, 0x00, 0x34, 0x4f, 0xa5, 0xa5}, &CmdVoidOutS64{})
	//struct test.CmdVoidOutS64_In { }
	binary.Register(binary.ID{0x94, 0x53, 0xd0, 0xdc, 0xd7, 0x4c, 0x8f, 0xc8, 0x92, 0xab, 0x9e, 0x8f, 0x86, 0xf6, 0xe4, 0x5f, 0x0d, 0xb0, 0xf7, 0x92}, &CmdVoidOutS64_In{})
	//struct test.CmdVoidOutS64_Out { A:int64 }
	binary.Register(binary.ID{0xa4, 0x92, 0x52, 0x7c, 0x76, 0xc7, 0x28, 0xd9, 0x5b, 0x93, 0x43, 0x8b, 0x16, 0xec, 0x9b, 0x46, 0xda, 0xb1, 0xf8, 0x8e}, &CmdVoidOutS64_Out{})
	//struct test.CmdVoidOutS8 { Context:atom.ContextID, In:CmdVoidOutS8_In, Out:CmdVoidOutS8_Out }
	binary.Register(binary.ID{0x32, 0x2b, 0xdf, 0xa9, 0x14, 0x93, 0x7e, 0x03, 0x19, 0x3f, 0x08, 0xf5, 0x8b, 0x20, 0x10, 0xff, 0xfc, 0xdf, 0xc6, 0x04}, &CmdVoidOutS8{})
	//struct test.CmdVoidOutS8_In { }
	binary.Register(binary.ID{0x64, 0x51, 0xa7, 0x31, 0xea, 0xa3, 0xd8, 0x1a, 0x29, 0xc2, 0x80, 0xf5, 0xf2, 0x3a, 0xf3, 0xd0, 0xcf, 0x97, 0x8c, 0xd5}, &CmdVoidOutS8_In{})
	//struct test.CmdVoidOutS8_Out { A:int8 }
	binary.Register(binary.ID{0x3f, 0xdc, 0xe4, 0x54, 0x18, 0x2f, 0x52, 0xb2, 0xa2, 0xc7, 0x55, 0xe7, 0xd8, 0x2f, 0x62, 0x7c, 0x79, 0xd8, 0xe5, 0x12}, &CmdVoidOutS8_Out{})
	//struct test.CmdVoidOutString { Context:atom.ContextID, In:CmdVoidOutString_In, Out:CmdVoidOutString_Out }
	binary.Register(binary.ID{0x14, 0x66, 0xca, 0x1e, 0x3a, 0x67, 0xcb, 0x47, 0x11, 0x36, 0x98, 0x8b, 0x64, 0xa9, 0x96, 0x4a, 0x75, 0x5f, 0xd7, 0x55}, &CmdVoidOutString{})
	//struct test.CmdVoidOutString_In { }
	binary.Register(binary.ID{0x8c, 0x64, 0xde, 0x14, 0x7c, 0x77, 0xb9, 0xb5, 0xda, 0x1d, 0x73, 0x73, 0x67, 0x88, 0x66, 0xdf, 0x19, 0xbe, 0x14, 0x49}, &CmdVoidOutString_In{})
	//struct test.CmdVoidOutString_Out { A:string }
	binary.Register(binary.ID{0xa0, 0x86, 0x50, 0x4d, 0x16, 0x5c, 0x18, 0x88, 0x68, 0xc0, 0x83, 0x79, 0x9d, 0x79, 0xbe, 0xea, 0xf6, 0x91, 0x8a, 0x6c}, &CmdVoidOutString_Out{})
	//struct test.CmdVoidOutU16 { Context:atom.ContextID, In:CmdVoidOutU16_In, Out:CmdVoidOutU16_Out }
	binary.Register(binary.ID{0xcd, 0x28, 0x5c, 0x10, 0x79, 0xdf, 0x33, 0xca, 0x4e, 0xd9, 0x74, 0xc8, 0x21, 0x8e, 0xed, 0x55, 0x51, 0xf6, 0xb6, 0x19}, &CmdVoidOutU16{})
	//struct test.CmdVoidOutU16_In { }
	binary.Register(binary.ID{0x9d, 0xd4, 0x7d, 0xf0, 0x72, 0x98, 0xca, 0x33, 0x02, 0xdf, 0x75, 0x5c, 0x0d, 0x96, 0xc2, 0x70, 0xdc, 0xd8, 0x31, 0x64}, &CmdVoidOutU16_In{})
	//struct test.CmdVoidOutU16_Out { A:uint16 }
	binary.Register(binary.ID{0xb5, 0x1d, 0x4c, 0x89, 0x7e, 0xa8, 0x7c, 0x44, 0x41, 0x3c, 0xf3, 0xfa, 0x25, 0x33, 0x58, 0x3b, 0x84, 0xe4, 0x5b, 0x68}, &CmdVoidOutU16_Out{})
	//struct test.CmdVoidOutU32 { Context:atom.ContextID, In:CmdVoidOutU32_In, Out:CmdVoidOutU32_Out }
	binary.Register(binary.ID{0xde, 0xae, 0xc8, 0x69, 0x2a, 0xf8, 0x11, 0x62, 0x06, 0x40, 0x90, 0xe4, 0x63, 0x78, 0xc5, 0xcd, 0x1a, 0x76, 0xb8, 0x4f}, &CmdVoidOutU32{})
	//struct test.CmdVoidOutU32_In { }
	binary.Register(binary.ID{0xb4, 0x3f, 0x2f, 0x02, 0x88, 0x18, 0xa7, 0xd8, 0xe8, 0x61, 0xc4, 0x4c, 0x23, 0xec, 0xc8, 0x37, 0xc9, 0x26, 0xf5, 0xda}, &CmdVoidOutU32_In{})
	//struct test.CmdVoidOutU32_Out { A:uint32 }
	binary.Register(binary.ID{0x6b, 0x9c, 0x03, 0xb9, 0x66, 0x45, 0x98, 0xbf, 0xfc, 0x2b, 0xe9, 0x23, 0x35, 0x19, 0xae, 0xfc, 0xe6, 0x96, 0xd0, 0x8a}, &CmdVoidOutU32_Out{})
	//struct test.CmdVoidOutU64 { Context:atom.ContextID, In:CmdVoidOutU64_In, Out:CmdVoidOutU64_Out }
	binary.Register(binary.ID{0x6f, 0xe9, 0x0c, 0x3b, 0x09, 0x95, 0x53, 0xdd, 0x03, 0x1e, 0x11, 0x08, 0x9c, 0x9a, 0xdc, 0x3d, 0x4e, 0x83, 0x49, 0x30}, &CmdVoidOutU64{})
	//struct test.CmdVoidOutU64_In { }
	binary.Register(binary.ID{0x65, 0x55, 0x3e, 0xf3, 0xdf, 0xb4, 0x25, 0x2c, 0x68, 0x21, 0xcd, 0x50, 0x18, 0x2e, 0xe3, 0x19, 0x48, 0xa5, 0xa3, 0x9e}, &CmdVoidOutU64_In{})
	//struct test.CmdVoidOutU64_Out { A:uint64 }
	binary.Register(binary.ID{0xd6, 0x7f, 0x83, 0x76, 0xfb, 0x02, 0xf0, 0x90, 0x4c, 0x98, 0xb5, 0x35, 0x8b, 0x40, 0xd2, 0xcc, 0xf7, 0xe9, 0x12, 0xf0}, &CmdVoidOutU64_Out{})
	//struct test.CmdVoidOutU8 { Context:atom.ContextID, In:CmdVoidOutU8_In, Out:CmdVoidOutU8_Out }
	binary.Register(binary.ID{0xd0, 0x8f, 0x08, 0x14, 0x71, 0x7f, 0xbc, 0x99, 0x21, 0x6e, 0xd6, 0x8d, 0x1f, 0xdd, 0x36, 0xfa, 0xbc, 0x3e, 0xb7, 0xe2}, &CmdVoidOutU8{})
	//struct test.CmdVoidOutU8_In { }
	binary.Register(binary.ID{0x3a, 0x86, 0xed, 0x4a, 0xe6, 0x08, 0xfb, 0xfb, 0x55, 0x60, 0x6d, 0xff, 0x8c, 0xdf, 0x97, 0x7a, 0x97, 0x05, 0x5a, 0xe0}, &CmdVoidOutU8_In{})
	//struct test.CmdVoidOutU8_Out { A:uint8 }
	binary.Register(binary.ID{0xcc, 0x4e, 0xe7, 0xc3, 0x4c, 0xc2, 0xda, 0xf0, 0x12, 0xba, 0x07, 0xc2, 0x33, 0xe6, 0xb9, 0x2a, 0x91, 0xf7, 0x37, 0x7b}, &CmdVoidOutU8_Out{})
	//struct test.CmdVoidS16 { Context:atom.ContextID, In:CmdVoidS16_In, Out:CmdVoidS16_Out }
	binary.Register(binary.ID{0x54, 0x5a, 0x6d, 0xc0, 0x63, 0x8e, 0x52, 0xad, 0x89, 0x9d, 0x0a, 0x7f, 0xc3, 0x0f, 0x05, 0xc5, 0xf4, 0xc0, 0xe5, 0xad}, &CmdVoidS16{})
	//struct test.CmdVoidS16_In { A:int16 }
	binary.Register(binary.ID{0x16, 0xae, 0xb7, 0xa3, 0x26, 0x62, 0x4c, 0xcb, 0xb5, 0xa5, 0xe7, 0x73, 0xe2, 0xae, 0x9f, 0x13, 0xa5, 0xab, 0xd6, 0x7b}, &CmdVoidS16_In{})
	//struct test.CmdVoidS16_Out { }
	binary.Register(binary.ID{0xc2, 0x8d, 0x3f, 0x6c, 0x18, 0xe3, 0xad, 0xcd, 0x1b, 0xfd, 0x32, 0x07, 0xb0, 0xac, 0xb6, 0x77, 0x61, 0xe2, 0x61, 0x2a}, &CmdVoidS16_Out{})
	//struct test.CmdVoidS32 { Context:atom.ContextID, In:CmdVoidS32_In, Out:CmdVoidS32_Out }
	binary.Register(binary.ID{0xab, 0x94, 0xfd, 0xb7, 0x22, 0x50, 0xe6, 0xca, 0x99, 0x13, 0x6e, 0xc1, 0x0f, 0x30, 0x35, 0xaf, 0xb0, 0x91, 0x1a, 0x90}, &CmdVoidS32{})
	//struct test.CmdVoidS32_In { A:int32 }
	binary.Register(binary.ID{0x54, 0xd9, 0xab, 0x22, 0xeb, 0x5e, 0xdd, 0xa3, 0x98, 0xa0, 0x9c, 0xeb, 0xa1, 0xbf, 0xce, 0x47, 0x4b, 0x1f, 0x1e, 0xb7}, &CmdVoidS32_In{})
	//struct test.CmdVoidS32_Out { }
	binary.Register(binary.ID{0x6a, 0x76, 0x11, 0x15, 0x8f, 0x47, 0xd6, 0x28, 0x77, 0x88, 0x92, 0x2d, 0x10, 0xf0, 0x35, 0xdc, 0xb6, 0xcb, 0x7e, 0xcd}, &CmdVoidS32_Out{})
	//struct test.CmdVoidS64 { Context:atom.ContextID, In:CmdVoidS64_In, Out:CmdVoidS64_Out }
	binary.Register(binary.ID{0x4c, 0x3c, 0x10, 0x60, 0x83, 0x3d, 0x2a, 0x95, 0xa5, 0x88, 0x4e, 0xb9, 0x4e, 0x95, 0x3e, 0x4f, 0x7c, 0x61, 0x0d, 0xcf}, &CmdVoidS64{})
	//struct test.CmdVoidS64_In { A:int64 }
	binary.Register(binary.ID{0x08, 0x8b, 0xf2, 0x89, 0xf3, 0xee, 0xb9, 0xde, 0x0c, 0xdc, 0x6f, 0x8c, 0x7d, 0xf9, 0x06, 0x8c, 0x8e, 0xca, 0x57, 0xc2}, &CmdVoidS64_In{})
	//struct test.CmdVoidS64_Out { }
	binary.Register(binary.ID{0x4f, 0xd2, 0x2a, 0x43, 0x41, 0x15, 0x7b, 0x52, 0x75, 0xd3, 0x17, 0xf3, 0xe8, 0x62, 0x9e, 0x24, 0xa6, 0x80, 0x43, 0xde}, &CmdVoidS64_Out{})
	//struct test.CmdVoidS8 { Context:atom.ContextID, In:CmdVoidS8_In, Out:CmdVoidS8_Out }
	binary.Register(binary.ID{0xd2, 0xb4, 0x40, 0x6d, 0xf4, 0x86, 0x47, 0x4d, 0xba, 0x24, 0xd3, 0x05, 0xc9, 0x23, 0x67, 0x74, 0x3a, 0xdc, 0xb5, 0xbe}, &CmdVoidS8{})
	//struct test.CmdVoidS8_In { A:int8 }
	binary.Register(binary.ID{0xb6, 0xca, 0xcf, 0x10, 0x86, 0x00, 0x71, 0x1f, 0x6b, 0x6d, 0x5c, 0x12, 0xe5, 0x22, 0xcd, 0x3a, 0xf6, 0xcc, 0xc3, 0xe4}, &CmdVoidS8_In{})
	//struct test.CmdVoidS8_Out { }
	binary.Register(binary.ID{0xdd, 0xf2, 0xef, 0x10, 0x58, 0xaf, 0x17, 0x18, 0x84, 0xed, 0x77, 0x36, 0xa7, 0xb9, 0xbc, 0x75, 0xe5, 0xf0, 0x43, 0x97}, &CmdVoidS8_Out{})
	//struct test.CmdVoidString { Context:atom.ContextID, In:CmdVoidString_In, Out:CmdVoidString_Out }
	binary.Register(binary.ID{0xf3, 0xc0, 0xeb, 0xea, 0x8f, 0xd2, 0x14, 0xd3, 0xa4, 0xe3, 0x6f, 0x4b, 0x26, 0xc3, 0xeb, 0x56, 0x4a, 0x6c, 0xf5, 0x36}, &CmdVoidString{})
	//struct test.CmdVoidString_In { A:string }
	binary.Register(binary.ID{0xb9, 0xab, 0x97, 0x3e, 0x7b, 0xcd, 0xa3, 0x51, 0x80, 0x02, 0x78, 0x72, 0xef, 0x8c, 0x09, 0x7a, 0x38, 0x8f, 0xab, 0xac}, &CmdVoidString_In{})
	//struct test.CmdVoidString_Out { }
	binary.Register(binary.ID{0xe5, 0x7d, 0x1d, 0x4b, 0x30, 0x25, 0x62, 0xd1, 0xcf, 0xd9, 0xd7, 0xc3, 0x90, 0x0d, 0x3e, 0xcb, 0xc3, 0xc0, 0xa5, 0x8b}, &CmdVoidString_Out{})
	//struct test.CmdVoidU16 { Context:atom.ContextID, In:CmdVoidU16_In, Out:CmdVoidU16_Out }
	binary.Register(binary.ID{0x8d, 0xb5, 0xb8, 0xdd, 0x67, 0xcd, 0xcf, 0x30, 0x2d, 0x86, 0xa5, 0xbf, 0x4e, 0xa7, 0x08, 0x20, 0xe9, 0x43, 0x78, 0xd4}, &CmdVoidU16{})
	//struct test.CmdVoidU16_In { A:uint16 }
	binary.Register(binary.ID{0x63, 0xc8, 0x6d, 0x49, 0x08, 0x06, 0x22, 0xd0, 0x35, 0xce, 0x67, 0xeb, 0x7c, 0x53, 0xbf, 0x45, 0xee, 0x5d, 0x46, 0x21}, &CmdVoidU16_In{})
	//struct test.CmdVoidU16_Out { }
	binary.Register(binary.ID{0x45, 0x1b, 0xd6, 0xc1, 0x77, 0x09, 0xd3, 0x30, 0x50, 0x4e, 0x96, 0x77, 0xbe, 0xfd, 0x47, 0xf5, 0xf8, 0xa9, 0x9e, 0xff}, &CmdVoidU16_Out{})
	//struct test.CmdVoidU32 { Context:atom.ContextID, In:CmdVoidU32_In, Out:CmdVoidU32_Out }
	binary.Register(binary.ID{0x50, 0x36, 0xdb, 0x4b, 0xb7, 0x63, 0xb5, 0xa5, 0xc2, 0x10, 0xa4, 0x7b, 0xbf, 0x27, 0xc7, 0xfd, 0xa9, 0xb8, 0x35, 0x61}, &CmdVoidU32{})
	//struct test.CmdVoidU32_In { A:uint32 }
	binary.Register(binary.ID{0x57, 0x0b, 0x28, 0xf4, 0xb9, 0x9f, 0x49, 0xc0, 0x61, 0x3c, 0x0f, 0x74, 0x64, 0x21, 0xb6, 0x36, 0x82, 0x54, 0x6c, 0xfb}, &CmdVoidU32_In{})
	//struct test.CmdVoidU32_Out { }
	binary.Register(binary.ID{0xb3, 0x89, 0x6a, 0x2f, 0x0b, 0x86, 0x82, 0x99, 0xfa, 0x6c, 0xc0, 0xfd, 0xa5, 0x3f, 0x9e, 0x54, 0xe8, 0x2d, 0x91, 0xf4}, &CmdVoidU32_Out{})
	//struct test.CmdVoidU64 { Context:atom.ContextID, In:CmdVoidU64_In, Out:CmdVoidU64_Out }
	binary.Register(binary.ID{0x64, 0x87, 0x14, 0xab, 0x1b, 0x8a, 0x7e, 0x95, 0xe4, 0x3f, 0x96, 0x63, 0x9d, 0xb8, 0xa1, 0x04, 0x38, 0xf6, 0x31, 0xdf}, &CmdVoidU64{})
	//struct test.CmdVoidU64_In { A:uint64 }
	binary.Register(binary.ID{0xff, 0x65, 0xf1, 0x19, 0xf1, 0x83, 0x1e, 0xa3, 0xec, 0xb7, 0xbf, 0x05, 0xd2, 0x01, 0x34, 0x49, 0x96, 0x24, 0xf5, 0x80}, &CmdVoidU64_In{})
	//struct test.CmdVoidU64_Out { }
	binary.Register(binary.ID{0xa9, 0xf2, 0xf5, 0x8f, 0xfa, 0x7c, 0xfa, 0xe6, 0x2a, 0xba, 0x4b, 0x9b, 0xc5, 0x66, 0xd0, 0x38, 0x5f, 0xfa, 0x45, 0x72}, &CmdVoidU64_Out{})
	//struct test.CmdVoidU8 { Context:atom.ContextID, In:CmdVoidU8_In, Out:CmdVoidU8_Out }
	binary.Register(binary.ID{0x10, 0x59, 0xf1, 0x81, 0x29, 0xee, 0x78, 0x8a, 0x88, 0xc3, 0xcc, 0xe1, 0xdb, 0x27, 0x92, 0x4d, 0x87, 0x7b, 0x1e, 0xa6}, &CmdVoidU8{})
	//struct test.CmdVoidU8_In { A:uint8 }
	binary.Register(binary.ID{0x71, 0xe9, 0x6a, 0x42, 0xf3, 0xb7, 0xb8, 0x7d, 0x2b, 0xbf, 0xc2, 0x6e, 0x90, 0xce, 0xfc, 0xf4, 0x8f, 0x9d, 0x95, 0x3b}, &CmdVoidU8_In{})
	//struct test.CmdVoidU8_Out { }
	binary.Register(binary.ID{0xfc, 0x5d, 0x21, 0xa4, 0x08, 0x80, 0x75, 0xbf, 0xd1, 0x06, 0xde, 0x62, 0x38, 0x89, 0xba, 0x82, 0x3d, 0xa7, 0xb2, 0x31}, &CmdVoidU8_Out{})
	//struct test.CmdVoid_In { }
	binary.Register(binary.ID{0xa5, 0x5f, 0x43, 0x9c, 0x2a, 0xa1, 0x4c, 0x57, 0xce, 0x2f, 0xa5, 0xe9, 0x24, 0xa1, 0x0e, 0x80, 0xc9, 0x4d, 0x50, 0x21}, &CmdVoid_In{})
	//struct test.CmdVoid_Out { }
	binary.Register(binary.ID{0x0a, 0x66, 0x32, 0xd7, 0x4e, 0x98, 0x3b, 0x60, 0x0c, 0x8c, 0x8e, 0x55, 0x3d, 0xeb, 0x45, 0x53, 0xa6, 0xce, 0x6a, 0xd1}, &CmdVoid_Out{})
	//struct test.Globals { }
	binary.Register(binary.ID{0xc0, 0x56, 0xa7, 0xd5, 0x53, 0x88, 0xac, 0xaa, 0x75, 0x81, 0x46, 0x04, 0x13, 0xd4, 0xd3, 0x88, 0x43, 0x9c, 0xef, 0x61}, &Globals{})
}

func (o CmdArrayOfFloat) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdArrayOfFloat) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdArrayOfFloat_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdArrayOfFloat_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdArrayOfFloat_Out) Encode(e *binary.Encoder) error {
	if err := e.Int32(int32(len(o.Result))); err != nil {
		return err
	}
	for i := range o.Result {
		if err := e.Float32(o.Result[i]); err != nil {
			return err
		}
	}
	return nil
}

func (o *CmdArrayOfFloat_Out) Decode(d *binary.Decoder) error {
	if count, err := d.Int32(); err != nil {
		return err
	} else {
		o.Result = make(F32Array, count)
		for i := range o.Result {
			if obj, err := d.Float32(); err != nil {
				return err
			} else {
				o.Result[i] = float32(obj)
			}
		}
	}
	return nil
}

func (o CmdBool) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdBool) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdBool_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdBool_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdBool_Out) Encode(e *binary.Encoder) error {
	if err := e.Bool(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdBool_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Bool(); err != nil {
		return err
	} else {
		o.Result = bool(obj)
	}
	return nil
}

func (o CmdF32) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdF32) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdF32_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdF32_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdF32_Out) Encode(e *binary.Encoder) error {
	if err := e.Float32(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdF32_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Float32(); err != nil {
		return err
	} else {
		o.Result = float32(obj)
	}
	return nil
}

func (o CmdF64) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdF64) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdF64_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdF64_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdF64_Out) Encode(e *binary.Encoder) error {
	if err := e.Float64(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdF64_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Float64(); err != nil {
		return err
	} else {
		o.Result = float64(obj)
	}
	return nil
}

func (o CmdPointer) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdPointer) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdPointer_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdPointer_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdPointer_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint64(uint64(o.Result)); err != nil {
		return err
	}
	return nil
}

func (o *CmdPointer_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint64(); err != nil {
		return err
	} else {
		o.Result = memory.Pointer(obj)
	}
	return nil
}

func (o CmdS16) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdS16) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdS16_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdS16_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdS16_Out) Encode(e *binary.Encoder) error {
	if err := e.Int16(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdS16_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Int16(); err != nil {
		return err
	} else {
		o.Result = int16(obj)
	}
	return nil
}

func (o CmdS32) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdS32) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdS32_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdS32_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdS32_Out) Encode(e *binary.Encoder) error {
	if err := e.Int32(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdS32_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Int32(); err != nil {
		return err
	} else {
		o.Result = int32(obj)
	}
	return nil
}

func (o CmdS64) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdS64) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdS64_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdS64_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdS64_Out) Encode(e *binary.Encoder) error {
	if err := e.Int64(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdS64_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Int64(); err != nil {
		return err
	} else {
		o.Result = int64(obj)
	}
	return nil
}

func (o CmdS8) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdS8) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdS8_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdS8_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdS8_Out) Encode(e *binary.Encoder) error {
	if err := e.Int8(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdS8_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Int8(); err != nil {
		return err
	} else {
		o.Result = int8(obj)
	}
	return nil
}

func (o CmdString) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdString) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdString_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdString_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdString_Out) Encode(e *binary.Encoder) error {
	if err := e.String(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdString_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.String(); err != nil {
		return err
	} else {
		o.Result = string(obj)
	}
	return nil
}

func (o CmdU16) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdU16) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdU16_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdU16_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdU16_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint16(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdU16_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint16(); err != nil {
		return err
	} else {
		o.Result = uint16(obj)
	}
	return nil
}

func (o CmdU32) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdU32) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdU32_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdU32_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdU32_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint32(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdU32_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Result = uint32(obj)
	}
	return nil
}

func (o CmdU64) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdU64) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdU64_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdU64_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdU64_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint64(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdU64_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint64(); err != nil {
		return err
	} else {
		o.Result = uint64(obj)
	}
	return nil
}

func (o CmdU8) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdU8) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdU8_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdU8_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdU8_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint8(o.Result); err != nil {
		return err
	}
	return nil
}

func (o *CmdU8_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint8(); err != nil {
		return err
	} else {
		o.Result = uint8(obj)
	}
	return nil
}

func (o CmdVoid) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoid) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoid3Arrays) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoid3Arrays) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoid3Arrays_In) Encode(e *binary.Encoder) error {
	if err := e.Int32(int32(len(o.A))); err != nil {
		return err
	}
	for i := range o.A {
		if err := e.Int8(o.A[i]); err != nil {
			return err
		}
	}
	if err := e.Int32(int32(len(o.B))); err != nil {
		return err
	}
	for i := range o.B {
		if err := e.String(o.B[i]); err != nil {
			return err
		}
	}
	if err := e.Int32(int32(len(o.C))); err != nil {
		return err
	}
	for i := range o.C {
		if err := e.Bool(o.C[i]); err != nil {
			return err
		}
	}
	return nil
}

func (o *CmdVoid3Arrays_In) Decode(d *binary.Decoder) error {
	if count, err := d.Int32(); err != nil {
		return err
	} else {
		o.A = make(S8Array, count)
		for i := range o.A {
			if obj, err := d.Int8(); err != nil {
				return err
			} else {
				o.A[i] = int8(obj)
			}
		}
	}
	if count, err := d.Int32(); err != nil {
		return err
	} else {
		o.B = make(StringArray, count)
		for i := range o.B {
			if obj, err := d.String(); err != nil {
				return err
			} else {
				o.B[i] = string(obj)
			}
		}
	}
	if count, err := d.Int32(); err != nil {
		return err
	} else {
		o.C = make(BoolArray, count)
		for i := range o.C {
			if obj, err := d.Bool(); err != nil {
				return err
			} else {
				o.C[i] = bool(obj)
			}
		}
	}
	return nil
}

func (o CmdVoid3Arrays_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoid3Arrays_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoid3Remapped) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoid3Remapped) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoid3Remapped_In) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.A)); err != nil {
		return err
	}
	if err := e.Uint32(uint32(o.B)); err != nil {
		return err
	}
	if err := e.Uint32(uint32(o.C)); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoid3Remapped_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.A = remapped(obj)
	}
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.B = remapped(obj)
	}
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.C = remapped(obj)
	}
	return nil
}

func (o CmdVoid3Remapped_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoid3Remapped_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoid3Strings) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoid3Strings) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoid3Strings_In) Encode(e *binary.Encoder) error {
	if err := e.String(o.A); err != nil {
		return err
	}
	if err := e.String(o.B); err != nil {
		return err
	}
	if err := e.String(o.C); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoid3Strings_In) Decode(d *binary.Decoder) error {
	if obj, err := d.String(); err != nil {
		return err
	} else {
		o.A = string(obj)
	}
	if obj, err := d.String(); err != nil {
		return err
	} else {
		o.B = string(obj)
	}
	if obj, err := d.String(); err != nil {
		return err
	} else {
		o.C = string(obj)
	}
	return nil
}

func (o CmdVoid3Strings_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoid3Strings_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidArrayOfStrings) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidArrayOfStrings) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidArrayOfStrings_In) Encode(e *binary.Encoder) error {
	if err := e.Int32(int32(len(o.A))); err != nil {
		return err
	}
	for i := range o.A {
		if err := e.String(o.A[i]); err != nil {
			return err
		}
	}
	return nil
}

func (o *CmdVoidArrayOfStrings_In) Decode(d *binary.Decoder) error {
	if count, err := d.Int32(); err != nil {
		return err
	} else {
		o.A = make(StringArray, count)
		for i := range o.A {
			if obj, err := d.String(); err != nil {
				return err
			} else {
				o.A[i] = string(obj)
			}
		}
	}
	return nil
}

func (o CmdVoidArrayOfStrings_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidArrayOfStrings_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidBool) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidBool) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidBool_In) Encode(e *binary.Encoder) error {
	if err := e.Bool(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidBool_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Bool(); err != nil {
		return err
	} else {
		o.A = bool(obj)
	}
	return nil
}

func (o CmdVoidBool_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidBool_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidF32) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidF32) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidF32_In) Encode(e *binary.Encoder) error {
	if err := e.Float32(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidF32_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Float32(); err != nil {
		return err
	} else {
		o.A = float32(obj)
	}
	return nil
}

func (o CmdVoidF32_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidF32_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidF64) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidF64) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidF64_In) Encode(e *binary.Encoder) error {
	if err := e.Float64(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidF64_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Float64(); err != nil {
		return err
	} else {
		o.A = float64(obj)
	}
	return nil
}

func (o CmdVoidF64_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidF64_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOut3Remapped) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOut3Remapped) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOut3Remapped_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOut3Remapped_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOut3Remapped_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.A)); err != nil {
		return err
	}
	if err := e.Uint32(uint32(o.B)); err != nil {
		return err
	}
	if err := e.Uint32(uint32(o.C)); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOut3Remapped_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.A = remapped(obj)
	}
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.B = remapped(obj)
	}
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.C = remapped(obj)
	}
	return nil
}

func (o CmdVoidOut3Strings) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOut3Strings) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOut3Strings_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOut3Strings_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOut3Strings_Out) Encode(e *binary.Encoder) error {
	if err := e.String(o.A); err != nil {
		return err
	}
	if err := e.String(o.B); err != nil {
		return err
	}
	if err := e.String(o.C); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOut3Strings_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.String(); err != nil {
		return err
	} else {
		o.A = string(obj)
	}
	if obj, err := d.String(); err != nil {
		return err
	} else {
		o.B = string(obj)
	}
	if obj, err := d.String(); err != nil {
		return err
	} else {
		o.C = string(obj)
	}
	return nil
}

func (o CmdVoidOutArrayOfRemapped) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutArrayOfRemapped) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutArrayOfRemapped_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutArrayOfRemapped_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutArrayOfRemapped_Out) Encode(e *binary.Encoder) error {
	if err := e.Int32(int32(len(o.A))); err != nil {
		return err
	}
	for i := range o.A {
		if err := e.Uint32(uint32(o.A[i])); err != nil {
			return err
		}
	}
	return nil
}

func (o *CmdVoidOutArrayOfRemapped_Out) Decode(d *binary.Decoder) error {
	if count, err := d.Int32(); err != nil {
		return err
	} else {
		o.A = make(RemappedArray, count)
		for i := range o.A {
			if obj, err := d.Uint32(); err != nil {
				return err
			} else {
				o.A[i] = remapped(obj)
			}
		}
	}
	return nil
}

func (o CmdVoidOutBool) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutBool) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutBool_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutBool_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutBool_Out) Encode(e *binary.Encoder) error {
	if err := e.Bool(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutBool_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Bool(); err != nil {
		return err
	} else {
		o.A = bool(obj)
	}
	return nil
}

func (o CmdVoidOutF32) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutF32) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutF32_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutF32_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutF32_Out) Encode(e *binary.Encoder) error {
	if err := e.Float32(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutF32_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Float32(); err != nil {
		return err
	} else {
		o.A = float32(obj)
	}
	return nil
}

func (o CmdVoidOutF64) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutF64) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutF64_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutF64_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutF64_Out) Encode(e *binary.Encoder) error {
	if err := e.Float64(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutF64_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Float64(); err != nil {
		return err
	} else {
		o.A = float64(obj)
	}
	return nil
}

func (o CmdVoidOutFixedSizeBuffer) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutFixedSizeBuffer) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutFixedSizeBuffer_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutFixedSizeBuffer_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutFixedSizeBuffer_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint64(uint64(o.A)); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutFixedSizeBuffer_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint64(); err != nil {
		return err
	} else {
		o.A = memory.Pointer(obj)
	}
	return nil
}

func (o CmdVoidOutS16) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutS16) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutS16_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutS16_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutS16_Out) Encode(e *binary.Encoder) error {
	if err := e.Int16(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutS16_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Int16(); err != nil {
		return err
	} else {
		o.A = int16(obj)
	}
	return nil
}

func (o CmdVoidOutS32) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutS32) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutS32_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutS32_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutS32_Out) Encode(e *binary.Encoder) error {
	if err := e.Int32(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutS32_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Int32(); err != nil {
		return err
	} else {
		o.A = int32(obj)
	}
	return nil
}

func (o CmdVoidOutS64) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutS64) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutS64_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutS64_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutS64_Out) Encode(e *binary.Encoder) error {
	if err := e.Int64(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutS64_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Int64(); err != nil {
		return err
	} else {
		o.A = int64(obj)
	}
	return nil
}

func (o CmdVoidOutS8) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutS8) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutS8_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutS8_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutS8_Out) Encode(e *binary.Encoder) error {
	if err := e.Int8(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutS8_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Int8(); err != nil {
		return err
	} else {
		o.A = int8(obj)
	}
	return nil
}

func (o CmdVoidOutString) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutString) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutString_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutString_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutString_Out) Encode(e *binary.Encoder) error {
	if err := e.String(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutString_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.String(); err != nil {
		return err
	} else {
		o.A = string(obj)
	}
	return nil
}

func (o CmdVoidOutU16) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutU16) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutU16_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutU16_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutU16_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint16(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutU16_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint16(); err != nil {
		return err
	} else {
		o.A = uint16(obj)
	}
	return nil
}

func (o CmdVoidOutU32) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutU32) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutU32_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutU32_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutU32_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint32(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutU32_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.A = uint32(obj)
	}
	return nil
}

func (o CmdVoidOutU64) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutU64) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutU64_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutU64_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutU64_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint64(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutU64_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint64(); err != nil {
		return err
	} else {
		o.A = uint64(obj)
	}
	return nil
}

func (o CmdVoidOutU8) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutU8) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidOutU8_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidOutU8_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidOutU8_Out) Encode(e *binary.Encoder) error {
	if err := e.Uint8(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidOutU8_Out) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint8(); err != nil {
		return err
	} else {
		o.A = uint8(obj)
	}
	return nil
}

func (o CmdVoidS16) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidS16) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidS16_In) Encode(e *binary.Encoder) error {
	if err := e.Int16(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidS16_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Int16(); err != nil {
		return err
	} else {
		o.A = int16(obj)
	}
	return nil
}

func (o CmdVoidS16_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidS16_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidS32) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidS32) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidS32_In) Encode(e *binary.Encoder) error {
	if err := e.Int32(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidS32_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Int32(); err != nil {
		return err
	} else {
		o.A = int32(obj)
	}
	return nil
}

func (o CmdVoidS32_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidS32_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidS64) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidS64) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidS64_In) Encode(e *binary.Encoder) error {
	if err := e.Int64(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidS64_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Int64(); err != nil {
		return err
	} else {
		o.A = int64(obj)
	}
	return nil
}

func (o CmdVoidS64_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidS64_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidS8) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidS8) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidS8_In) Encode(e *binary.Encoder) error {
	if err := e.Int8(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidS8_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Int8(); err != nil {
		return err
	} else {
		o.A = int8(obj)
	}
	return nil
}

func (o CmdVoidS8_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidS8_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidString) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidString) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidString_In) Encode(e *binary.Encoder) error {
	if err := e.String(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidString_In) Decode(d *binary.Decoder) error {
	if obj, err := d.String(); err != nil {
		return err
	} else {
		o.A = string(obj)
	}
	return nil
}

func (o CmdVoidString_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidString_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidU16) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidU16) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidU16_In) Encode(e *binary.Encoder) error {
	if err := e.Uint16(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidU16_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint16(); err != nil {
		return err
	} else {
		o.A = uint16(obj)
	}
	return nil
}

func (o CmdVoidU16_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidU16_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidU32) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidU32) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidU32_In) Encode(e *binary.Encoder) error {
	if err := e.Uint32(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidU32_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.A = uint32(obj)
	}
	return nil
}

func (o CmdVoidU32_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidU32_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidU64) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidU64) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidU64_In) Encode(e *binary.Encoder) error {
	if err := e.Uint64(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidU64_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint64(); err != nil {
		return err
	} else {
		o.A = uint64(obj)
	}
	return nil
}

func (o CmdVoidU64_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidU64_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoidU8) Encode(e *binary.Encoder) error {
	if err := e.Uint32(uint32(o.Context)); err != nil {
		return err
	}
	if err := o.In.Encode(e); err != nil {
		return err
	}
	if err := o.Out.Encode(e); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidU8) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint32(); err != nil {
		return err
	} else {
		o.Context = atom.ContextID(obj)
	}
	if err := o.In.Decode(d); err != nil {
		return err
	}
	if err := o.Out.Decode(d); err != nil {
		return err
	}
	return nil
}

func (o CmdVoidU8_In) Encode(e *binary.Encoder) error {
	if err := e.Uint8(o.A); err != nil {
		return err
	}
	return nil
}

func (o *CmdVoidU8_In) Decode(d *binary.Decoder) error {
	if obj, err := d.Uint8(); err != nil {
		return err
	} else {
		o.A = uint8(obj)
	}
	return nil
}

func (o CmdVoidU8_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoidU8_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoid_In) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoid_In) Decode(d *binary.Decoder) error {
	return nil
}

func (o CmdVoid_Out) Encode(e *binary.Encoder) error {
	return nil
}

func (o *CmdVoid_Out) Decode(d *binary.Decoder) error {
	return nil
}

func (o Globals) Encode(e *binary.Encoder) error {
	return nil
}

func (o *Globals) Decode(d *binary.Decoder) error {
	return nil
}
